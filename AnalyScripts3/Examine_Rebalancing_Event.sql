...
station_diff_201510_201609_killnull_yesservice_nodup.start_time, 
station_diff_201510_201609_killnull_yesservice_nodup.end_time, 
station_diff_201510_201609_killnull_yesservice_nodup.dock_id, 
station_diff_201510_201609_killnull_yesservice_nodup.delta
FROM station_diff_201510_201609_killnull_yesservice_nodup
JOIN Trip1015_0916
...
-- Try to calculat delta using Sep. data only
...
count(*) as outflow, 
station_outtrip09_timeperiod_1.delta as station_delta
FROM station_outtrip09_timeperiod_1
GROUP BY dock_id, start_time, end_time, delta;
...
count(*) as inflow, 
station_intrip09_timeperiod.delta as station_delta
FROM station_intrip09_timeperiod
GROUP BY dock_id, start_time, end_time, delta;

-----------------------------------
--Compute the delta of (inflow- outflow) from trip data and match with the delta from station data
create table intrip09_tripdelta_v2
AS SELECT
...
outtrip09_timeperiod_count.station_delta,
(intrip09_timeperiod_count.inflow - outtrip09_timeperiod_count.outflow) as trip_delta
...
------------------------------------------------
--Examine the delta matching 
select count (*) from intrip09_tripdelta_v2 WHERE (station_delta - trip_delta == 0);
>  21679 
select count (*) from intrip09_tripdelta_v2;
> 130776

select in_dock_id, in_start_time, in_end_time from intrip09_tripdelta_v2 WHERE (abs(station_delta - trip_delta) >= 5) limit 50;
